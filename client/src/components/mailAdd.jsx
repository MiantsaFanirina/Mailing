...
const MailAdd = () => {
    ...
    const [currentMail, setCurrentMail] = useState({
        subject: '',
        sender: { name: "", address: "", phoneNumber: "" },
        receiver: { name: "", address: "", phoneNumber: "" },
        sendDate: {
            date: now.toISOString().split('T')[0],
            time: now.toTimeString().slice(0, 5) // Format HH:MM
        },
        receiveDate: {
            date: thirtyMinutesLater.toISOString().split('T')[0],
            time: thirtyMinutesLater.toTimeString().slice(0, 5) // Format HH:MM
        },
        status: "",
        factor: { id: "" },
    });
    ...
    return (
        <dialog id={"addMailModal"} className="modal">
            <div className="modal-box md:p-12">
                ...
                {/* Postman */}
                <div className="label">
                    <span className="label-text">Mes facteurs disponibles</span>
                </div>
                <select
                    value={currentMail.status}
                    onChange={(e) => handleInputChange(e, "status")}
                    className="select select-bordered select-sm w-full"
                >
                    <option value="delivered">F023 Rakotondrafara Miantsa Fanirina</option>
                    <option value="delivered">F023 Rakotondrafara Miantsa Fanirina</option>
                    <option value="delivered">F023 Rakotondrafara Miantsa Fanirina</option>
                    <option value="delivered">F023 Rakotondrafara Miantsa Fanirina</option>
                    <option value="delivered">F023 Rakotondrafara Miantsa Fanirina</option>
